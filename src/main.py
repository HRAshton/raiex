...
from raif_client.grid_parsers import init_grid_parsers
...
def _get_transactions() -> list[dict]:
    ...
    init_grid_parsers(system_parameters)

    login(session, environ["RAIF_USERNAME"], environ["RAIF_PASSWORD"])
    logging.info("Login successful")

    accounts = get_all_account_balance(session)
    logging.info(f"Found {len(accounts)} accounts")

    flat_transactions = []
    for account in accounts:
        turnover = get_transactional_account_turnover(session, account)
